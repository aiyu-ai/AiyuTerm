...
// Original file: Sources/CodeIsland/PixelCharacterView.swift (379 lines)
// Copyright (c) 2026 wxtsky — MIT License
// Modifications (c) 2026 AiyuAI — Apache License 2.0
//
// Clawd — the Claude pixel-art mascot. Ported 1:1 from the upstream
// SVG-derived Canvas rendering. The only AiyuTerm-local changes are:
//   • Type renamed `ClawdView` -> `AgentClawdView`
//   • Status enum switched from `AgentStatus` to `AgentHookStatus`
//     (AiyuTerm's hook-driven status enum lives in
//     `AgentHookModels.swift` and matches upstream 1:1)
//   • Mascot speed environment key is `pixelKitMascotSpeed`
//     (defined in `AgentPixelKitEnvironment.swift`)
//
...
struct AgentClawdView: View {
    let status: AgentHookStatus
    var size: CGFloat = 27
    @State private var alive = false
    @Environment(\.pixelKitMascotSpeed) private var speed
