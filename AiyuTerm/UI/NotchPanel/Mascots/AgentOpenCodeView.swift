//
// AgentOpenCodeView.swift
// AiyuTerm
//
// Phase 11.3A — CodeIsland mascot port.
// OpenCode terminal block mascot.
//
// Adapted 1:1 from CodeIsland upstream `OpenCodeView.swift`.
// All shapes, colors, and timing values are preserved. The only changes
// are: upstream `AgentStatus` -> Aiyu `AgentSessionStatus`, and
// `\.mascotSpeed` -> `\.agentMascotSpeed` (the enum's timeMultiplier
// is used where upstream multiplied by a Double).
//
...
struct AgentOpenCodeView: View {
    ...
    private var sleepScene: some View {
        ZStack {
            TimelineView(.periodic(from: .now, by: 0.06)) { ctx in
                sleepCanvas(t: ctx.date.timeIntervalSinceReferenceDate * speed.timeMultiplier)
            }
            TimelineView(.periodic(from: .now, by: 0.05)) { ctx in
                floatingZs(t: ctx.date.timeIntervalSinceReferenceDate * speed.timeMultiplier)
            }
        }
    }
    ...
    private var workScene: some View {
        TimelineView(.periodic(from: .now, by: 0.03)) { ctx in
            workCanvas(t: ctx.date.timeIntervalSinceReferenceDate * speed.timeMultiplier)
        }
    }
    ...
    private var alertScene: some View {
        ZStack {
            ...
            TimelineView(.periodic(from: .now, by: 0.03)) { ctx in
                alertCanvas(t: ctx.date.timeIntervalSinceReferenceDate * speed.timeMultiplier)
            }
        }
    }
