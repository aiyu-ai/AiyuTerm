...
// Codex cloud + prompt mascot.
//
// Adapted 1:1 from CodeIsland upstream `DexView.swift`.
...
struct AgentDexView: View {
    ...
    private var workScene: some View {
        TimelineView(.periodic(from: .now, by: 0.03)) { timeline in
            let t = timeline.date.timeIntervalSinceReferenceDate * speed.timeMultiplier
            workCanvas(t: t)
        }
    }
